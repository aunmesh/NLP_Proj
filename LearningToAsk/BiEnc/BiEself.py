...
class EncoderLSTM(nn.Module):
    ...
    def forward(self, x, initial_state):
        ...
        # [batch_size, num_layers * num_directions , 2 * hidden_size ]
        h_n_fixed = h_n.transpose(0,1).contiguos()
        c_n_fixed = c_n.transpose(0,1).contiguos()

        if isinstance(output, PackedSequence):
            output, output_lengths = pad_packed_sequence(output) # output is of size [Max_seq_length , batch , 2 * hidden_size]
